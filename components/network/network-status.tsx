...
import { Activity, Clock, Fuel, Blocks, Server, Zap, TrendingUp, CheckCircle2, Wifi, Cpu } from "lucide-react"
...
function GasPriceChartNetwork() {
  ...
  return (
    <div>
      {/* Chart */}
      <div className="relative h-40 rounded-lg border border-white/[0.05] bg-white/[0.02] p-4">
          ...
          {historicalData.map((point, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Tooltip */}
                <div className="pointer-events-none absolute -top-20 left-1/2 z-10 hidden -translate-x-1/2 whitespace-nowrap rounded-lg border border-white/[0.1] bg-slate-900/95 px-3 py-2 text-xs shadow-lg group-hover:block">
                  <p className="font-mono text-cyan-400">{point.gasPrice.toFixed(7)} INJ</p>
                  <p className="text-slate-400">{point.time}</p>
                  <p className="text-xs text-slate-500">{point.minutesAgo}m ago</p>
                </div>
              </div>
            )
          })}
      ...
      <div className="mt-4 flex items-center justify-between rounded-lg bg-white/[0.02] border border-white/[0.05] p-3">
        ...
        <span className="font-mono font-medium text-cyan-400">{avgGasPrice.toFixed(7)} INJ</span>
      </div>
    </div>
  )
}
...
export function NetworkStatus() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Gas Price Trend with Real Data */}
      <FuturisticCard glowColor="magenta" delay={0.4}>
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-emerald-400">Live: ${networkData?.injPrice?.toFixed(2) || "4.61"}/INJ</span>
          </div>
        </div>

        <p className="mb-4 text-sm text-muted-foreground">
          {networkData?.trend === "stable" 
            ? "Gas prices are stable. Network conditions are normal."
            : "Current network conditions"}
        </p>

        <GasPriceChartNetwork />
      </FuturisticCard>
